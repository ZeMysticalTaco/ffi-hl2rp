...
ITEM.functions.Customize = { -- sorry, for name order.
	name = "Customize",
	tip = "Customize your weapon.",
	icon = "icon16/wrench.png",
	OnRun = function(item)
		item:Customize(item.player)
		print("test")
		return false
	end
}

function ITEM:Customize(ply)
	if self:GetData("inventory_id", nil) then
	else
		print(ply)
		--[[ix.item.NewInv(ply:GetCharacter():GetID(), "customizedweapon", function(inv)
			self:SetData("inventory_id", inv:GetID())
			print("gay")
		end)--]]
	end
end
